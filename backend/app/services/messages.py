...
def closed_text(sent_text: str, occurrence: Occurrence, tz: ZoneInfo) -> str:
    """Погашенное сообщение: то, что отправляли, плюс итог.

    Собирается в одном месте намеренно. Гасят сообщения двое — бот сразу после
    нажатия и джоб закрытия (шаг 6), — и если склеивать текст у каждого своим
    способом, одно и то же занятие будет выглядеть в чате по-разному в
    зависимости от того, откуда пришло действие. Ровно от этого §9.

    sent_text — снимок из sent_messages, а не пересобранный reminder/followup:
    состояние занятия к этому моменту уже другое, см. докстринг модели.
    """
    return f"{sent_text}\n\n{action_note(occurrence, tz)}"
