...
**GitHub PlantUML в markdown не рендерит** — блоки ниже видны как исходник. Это осознанная плата за нотацию: цена в том, что диаграммы не видно прямо в вебе GitHub. На PlantUML переведён весь `docs/`, [`erd.md`](erd.md) в том числе, чтобы формат в проекте был один. Смотреть картинками:
...
System_Boundary(compose, "docker compose · локальная машина") {
    ...
    Container_Boundary(image, "один образ backend/ · общие models/ и services/") {
        ...
        Container(worker, "worker", "APScheduler", "Четыре джоба.\nТолько исходящие вызовы Telegram")
...
Rel(worker, tg, "Шлёт уведомления, гасит отвеченные", "sendMessage / editMessageText · исходящий")
...
Из этого же следует, почему гашение кнопок в отвеченных сообщениях (шаг 6, §6.4) сделано **джобом в воркере**, а не вызовом из роутера API: иначе с Telegram начали бы разговаривать три процесса вместо двух. Направление при этом не изменилось — воркер по-прежнему только зовёт Telegram сам, `editMessageText` такой же исходящий вызов, как и `sendMessage`.

### Диспетчер сначала отправляет и только потом ставит `notified_at`

Порядок именно такой, и он выглядит неправильным ([jobs.py:137-143](../backend/app/scheduler/jobs.py#L137-L143)).
...
Отдельно: отметка не проставляется и при обычной ошибке отправки — `_deliver` возвращает `False`, и occurrence остаётся в прежнем статусе до следующего тика ([jobs.py:37-73](../backend/app/scheduler/jobs.py#L37-L73)). Одна недоставленная запись не роняет остальную пачку.

Тот же порядок и та же причина у гашения сообщений (§6.4): сначала правим чат, потом ставим `closed_at`. Наоборот отметка стояла бы у сообщения, которое так и осталось с живыми кнопками, и повторить было бы уже некому.
...
Нарисованы пять сценариев, где участников больше одного и порядок неочевиден. CRUD-ручки, снуз, статистика и heatmap диаграммами не проясняются — что не нарисовано и почему, перечислено в конце.
...
loop каждые SCHEDULER_TICK_SECONDS
    ...
    DB --> W: пачка занятий
    opt занятие вернул сюда снуз — прошлые сообщения вытеснены
        W -> N: close(user, message_id, «⏰ Перенесено на 19:05»)
        N -> TG: editMessageText, кнопки убраны
        W -> DB: closed_at=now у прошлых sent_messages
    end
    W -> N: send(user, текст, кнопки)
    N -> TG: sendMessage + inline-клавиатура
    N --> W: message_id
    W -> DB: mark_notified: status=notified, notified_at=now
    W -> DB: sent_messages: kind=notification,\nmessage_id, снимок текста
    ...
    W -> DB: sent_messages: kind=followup,\nmessage_id, снимок текста
...
- **`send` идёт раньше `mark_notified` намеренно.** Обоснование — в «Диспетчер сначала отправляет» выше. Если `sendMessage` упал, отметка не выполняется и запись достаётся следующему тику. Номерами шагов эти отсылки не даются намеренно: `autonumber` пересчитывает их при любой правке диаграммы, и однажды они уже разъехались с текстом.
...
- **Текст в `sent_messages` кладётся тот же, что ушёл в чат.** Не ссылка на способ его собрать: к моменту гашения занятие уже в другом статусе, и пересобранный текст отличался бы от отправленного (§3, `SentMessage`). Отсюда же `opt`-блок: вернувшееся после снуза занятие сначала гасит прошлые сообщения — итог там считается, пока статус ещё `snoozed`, иначе получилось бы «Статус: notified» вместо «Перенесено на 19:05».
...
- **Сообщение старше 48 часов Telegram не даёт редактировать.** Такое приходит как `InaccessibleMessage`, правка тихо пропускается — но переход статуса уже закоммичен, отказываться от него было бы хуже.
- **Бот гасит только то сообщение, на котором нажали.** Второе сообщение того же занятия (первое уведомление и догоняющий пинг живут в чате одновременно) достаётся джобу §6.4 — см. сиквенс 5. Текст для правки бот берёт снимком из `sent_messages`, тем же самым, что и джоб: иначе одно занятие выглядело бы в чате по-разному в зависимости от того, откуда пришло действие.
...
### 5. Действие в вебе гасит сообщение в чате

Находка 2 и весь смысл шага 6. Участников здесь больше, чем в любом другом
сценарии, и главное в нём — что процесса **два** и они не знают друг о друге.

```plantuml
@startuml web-to-chat
autonumber
hide footbox
title Синхронизация чата с действием в вебе · §6.4

actor "Пользователь" as U
participant "web" as FE
participant "api" as API
database "PostgreSQL" as DB
participant "worker · джоб закрытия сообщений" as W
participant "Telegram Bot API" as TG

U -> FE: жмёт «Выполнил» на «Сегодня»
FE -> API: POST /occurrences/{id}/complete
API -> DB: occurrences.complete: status=done,\nfinished_at=now + COMMIT
API --> FE: 200, обновлённая карточка
note over API, TG
  api Telegram не трогает вовсе:
  воркер отправляет, бот принимает (§2)
end note

... до одного тика ...

W -> DB: незакрытые sent_messages\nу занятий в done/skipped/missed
DB --> W: две строки: уведомление и пинг
loop по каждой строке
    W -> W: снимок текста + «✅ Выполнено в 19:34»
    W -> TG: editMessageText, кнопки убраны
    W -> DB: closed_at = now
end
W -> DB: COMMIT
@enduml
...
- **Между `api` и `worker` нет ни одной связи** — ни вызова, ни очереди. Единственное, что их связывает, — строка в БД, которую один изменил, а другой увидел. Отсюда и задержка до тика: это не недоработка, а цена того же решения, по которому планировщик вынесен из `api`.
- **Диаграмма одинаково верна для трёх источников действия.** Замените первые три шага на нажатие кнопки в чате или на ночной джоб — остальное не изменится: джоб смотрит только на статус занятия. Ровно поэтому чинить находку 2 отдельно «для веба» было не нужно.
- **Сообщений именно два** — первое уведомление и догоняющий пинг, они живут в чате одновременно (§5). Нажатие в чате гасит только то сообщение, на котором нажали: остальные достаются этому джобу.
- **Итог зависит от статуса, а исходный текст — нет.** Первый берётся у занятия в момент гашения (`messages.action_note`), второй — снимком из `sent_messages`. Смешивать нельзя: пересобранный текст соответствовал бы уже другому состоянию.
